...
- **Database:** Migration `20260601190000_per_tenant_paystack` backfills `WebhookEvent.tenant_id` from order metadata, flips it `NOT NULL`, drops the old unique `(provider, event_type, reference)`, adds new unique `(tenant_id, provider, event_type, reference)`.
...
- Form save with `paystackDepositEnabled: true` for a tenant without keys → 400 with deep link to Integrations.
