# Documentation Index

Welcome to the E-Commerce COD Admin Dashboard documentation.

## Quick Links

- **New to the project?** Start with [Getting Started](guides/GETTING_STARTED.md)
- **Quick setup?** See [Quick Start](guides/QUICK_START.md)
- **Developer?** Read [Developer Guide](development/DEVELOPER_GUIDE.md)
- **Deploying?** Check [Deployment Guide](deployment/DEPLOYMENT_GUIDE.md)

## Documentation Structure

### 📖 Guides (User-Facing)

- [Getting Started](guides/GETTING_STARTED.md) - Complete onboarding guide
- [Quick Start](guides/QUICK_START.md) - Fast setup for experienced users
- [User Guide](guides/USER_GUIDE.md) - Feature walkthroughs and usage
- [Webhook Integration](guides/WEBHOOK_INTEGRATION_GUIDE.md) - External integrations
- [Workflow Automation](guides/WORKFLOW_AUTOMATION_GUIDE.md) - Automation setup

### 💻 Development

- [API Documentation](development/API_DOCUMENTATION.md) - REST API reference
- [Developer Guide](development/DEVELOPER_GUIDE.md) - Architecture and patterns
- [Testing Setup](development/TESTING_SETUP_SUMMARY.md) - Test infrastructure
- [Security Guide](development/SECURITY_GUIDE.md) - Security best practices
- [Contributing](development/CONTRIBUTING.md) - Contribution guidelines
- [FAQ](development/FAQ.md) - Frequently asked questions

### 🚀 Deployment

- [Deployment Guide](deployment/DEPLOYMENT_GUIDE.md) - Deployment instructions
- [Deployment Options](deployment/DEPLOYMENT_OPTIONS.md) - Hosting options
- [Docker Setup](deployment/DOCKER_SETUP.md) - Docker configuration
- [Production Checklist](deployment/PRODUCTION_CHECKLIST.md) - Pre-launch checklist
- [DevOps Guide](deployment/DEVOPS.md) - CI/CD and infrastructure

### Features

- [Feature Index](features/README.md) — Documentation for all feature areas
- Each feature gets one file; related Linear issues append as work progresses

### Work History

- [Feature Docs](features/) — Implementation history organized by feature
- [CHANGELOG](../CHANGELOG.md) — Version history

### 📦 Archive

Historical completion reports and summaries:
- Project completion reports
- Phase summaries
- Performance reports
- Feature implementation logs

See [archive/](archive/) for full list.

## Additional Resources

- [CLAUDE.md](../CLAUDE.md) - Claude Code AI assistant guidance
- [CHANGELOG.md](../CHANGELOG.md) - Version history
- [Backend Docs](../backend/) - Backend-specific documentation
- [Frontend Docs](../frontend/) - Frontend-specific documentation
