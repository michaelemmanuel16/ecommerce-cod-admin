  ...
  const loadProduct = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        costPrice: '', // Product type doesn't have costPrice
              ...
              <div className="flex items-center pt-7 px-3 py-2 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <span className="text-sm font-semibold text-green-600">
                  {formData.price && formData.costPrice
                    ? `$${(parseFloat(formData.price) - parseFloat(formData.costPrice)).toFixed(2)} (${(((parseFloat(formData.price) - parseFloat(formData.costPrice)) / parseFloat(formData.price)) * 100).toFixed(1)}%)`
                    : '—'}
                </span>
              </div>
            </div>
