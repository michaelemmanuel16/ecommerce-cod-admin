...
export const McpIntegration: React.FC = () => {
  ...
  const [configTab, setConfigTab] = useState<'remote' | 'local'>('remote');
              ...
              {!generatedKey ? (
                ...
                <>
                  ...
                  <div className="flex gap-1 mb-2">
                    <button
                      onClick={() => { setConfigTab('remote'); setCopiedConfig(false); }}
                      className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${configTab === 'remote' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-100'}`}
                    >
                      Remote (recommended)
                    </button>
                    <button
                      onClick={() => { setConfigTab('local'); setCopiedConfig(false); }}
                      className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${configTab === 'local' ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-100'}`}
                    >
                      Local (stdio)
                    </button>
                  </div>
...
{JSON.stringify(generatedKey.mcpConfig[configTab], null, 2)}
                    </pre>
                    <button
                      onClick={() => copyToClipboard(JSON.stringify(generatedKey.mcpConfig[configTab], null, 2), 'config')}
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                    ...
                    <ol className="text-xs text-blue-700 space-y-1 list-decimal list-inside">
                      ...
                      <li>Paste the {configTab === 'remote' ? 'remote' : 'local'} config JSON above</li>
